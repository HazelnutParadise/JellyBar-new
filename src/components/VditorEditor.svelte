<script>
  ...
  const loadResources = () => {
    return new Promise((resolve) => {
      // 檢查是否已載入
      if (document.querySelector('#vditor-css') && document.querySelector('#vditor-js')) {
        resolve();
        return;
      }
      ...
      css.id = 'vditor-css';
      css.rel = 'stylesheet';
      css.href = 'https://cdn.jsdelivr.net/npm/vditor@3.9.6/dist/index.css';
      document.head.appendChild(css);

      // 載入 JS
      const script = document.createElement('script');
      script.id = 'vditor-js';
      script.src = 'https://cdn.jsdelivr.net/npm/vditor@3.9.6/dist/index.min.js';
